...
final class WeatherServiceTests: XCTestCase {
    ...
    private func date(_ yyyymmdd: String) throws -> Date {
        let formatter = DateFormatter()
        formatter.dateFormat = "yyyy-MM-dd"
        return try XCTUnwrap(formatter.date(from: yyyymmdd))
    }

    func testMakeWeatherDataParsesCurrentConditions() throws {
        let response = try decode(WeatherFixture.json())
        let data = try XCTUnwrap(try WeatherService.makeWeatherData(
            from: response,
            now: date("2026-05-31")
        ))
    ...
    func testMakeWeatherDataReturnsNilWhenCurrentConditionMissing() throws {
        ...
        XCTAssertNil(WeatherService.makeWeatherData(from: response))
    }
